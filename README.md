...
다중 개발 서버 구축 가이드를 **AWS에서 실제로 빌드 가능한 레퍼런스 구현**으로 옮긴 저장소입니다.
...
│   ├── MULTI_BETA_GUIDE.md       # 멀티베타환경 이론 + Mermaid 다이어그램
│   ├── DEVELOPMENT.md            # 로컬/CI 개발 컨벤션
...
| `OWNER/REPO`      | `blue45f/multi-environment-setting` | GitHub 저장소                                                                                                 | `variables.tf`(OIDC sub)                                      |
...
- [docs/MULTI_BETA_GUIDE.md](docs/MULTI_BETA_GUIDE.md) — 멀티베타환경 이론, Mermaid 다이어그램, 운영 싱크 체크리스트
...
## 9. 최근 배포/개선 로그 (2026-06-11 기준)

최근 데모 목적의 소개 페이지를 다음 기준으로 고도화했습니다.

1. 소개 페이지 완전 리라이팅

- `/intro`를 멀티베타환경 onboarding 관점으로 재설계했습니다.
- 아키텍처 흐름을 단계별로 도식화(`1~5` 단계)해 코드 변경 → 정적 빌드 → runtime 설정 주입 → 환경별 라우팅 순서를 한 눈에 파악할 수 있게 구성했습니다.
- README/ENVIRONMENTS/runbook과 같은 용어를 쓰도록 source-of-truth, Markdown theory notes, Mermaid source, 가이드 싱크표를 추가했습니다.
...
- `docs/MULTI_BETA_GUIDE.md`를 추가해 build once, route/prefix/domain, runtime config, promotion/rollback 이론을 Mermaid 다이어그램으로 설명했습니다.
- 소개페이지의 diagram source와 Markdown 문서의 다이어그램을 같은 모델로 맞췄습니다.
- `SITE_URL` 기준 canonical URL을 대표 Vercel alias인 `https://multi-beta-guide.vercel.app`로 갱신했습니다.
...
- 한글/URL 줄바꿈 안정성을 위해 주요 UI 자간을 0으로 정리했습니다.
...
- Vercel 대표 URL은 개인 프로젝트 공유와 포트폴리오 공개용으로 유지하고, AWS S3/CloudFront 문서는 실제 운영형 멀티환경 기준으로 유지합니다.
...
- Vercel 대표 URL: `https://multi-beta-guide.vercel.app`
- Vercel 소개 페이지: `https://multi-beta-guide.vercel.app/intro/`
- Vercel 런타임 설정: `https://multi-beta-guide.vercel.app/env.json`
- Vercel legacy URL: `https://web-blond-nine-45.vercel.app`
